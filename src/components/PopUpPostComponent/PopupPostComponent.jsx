    ...
    const submitHandler = async (e) => {
        ...
        try {
          ...
          if (file) {
            ...
            alert("Success", "Post has been created", 3000);
            navigate("/home");
          } else {
          ...
          alert("Warning", "Photo is required", 3000);
        }
      };
        ...
        <div id="maindiv" onClick={props.onClose} className ="app z-[10000] fixed flex min-h-full min-w-full flex-col justify-center overflow-hidden bg-black bg-opacity-75 py-6 sm:py-12 opacity-up">
            <div className="w-7/12 mx-auto bg-white p-4 rounded-lg shadow-md slide-up ">
                ...
                <div className="flex space-x-2">
                    <input type="file" id="file" accept=".png, .jpeg, .jeg" onChange={(e)=> setFile(e.target.files[0])} className="flex items-center space-x-1 text-red-500 hover:bg-gray-100 p-2 rounded-lg hidden"/>
                    <label htmlFor="file" className="flex items-center space-x-1 text-red-500 hover:bg-gray-100 p-2 rounded-lg cursor-pointer">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.94 5H13v7h-2v-7H4.06L9 10l3-4 3 4z"></path></svg>
                        <span>Photo</span>
                    </label>
                </div>
